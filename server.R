...
shinyServer(function(input, output,session) {
  ...
  output$parentPlot<-renderPlot({
      ...
      if (dist=="Uniform"){
          ...
          #draw the parent distribution plot
          plot(x=x,y=dgamma(x=x,shape=alpha,rate=lambda),main=paste(dist," Density"),
               xlab="y", ylab="f(y)",type="l")
      } else if (dist=="Exponential") {
          #parameters
          lambda<-input$param1
          ...
          #draw the parent distribution plot
          plot(x=x,y=dexp(x=x,rate=lambda),main=paste(dist," Density"),
               xlab="y", ylab="f(y)",type="l")
      } else if (dist=="Chi-Square") {
          #parameters
          df<-input$param1
          ...
          #draw the parent distribution plot
          plot(x=x,y=dchisq(x=x,df=df),main=paste(dist," Density"),
               xlab="y", ylab="f(y)",type="l")
      } else if (dist=="Beta") {
          #Plotting sequence
          ...
          beta<-input$param2
          #draw the parent distribution plot
          plot(x=x,y=dbeta(x=x,shape1=alpha,shape2=beta),main=paste(dist," Density"),
               xlab="y", ylab="f(y)",type="l")
      } else if (dist=="Binomial") {
          #Plotting sequence
...
#           if (prob>=0.5){
#               x <- seq(from=0,to=ceiling(1/prob+6*(1-prob)/prob^2),by=1)
#           } else {
#               x <- seq(from=0,to=ceiling(1/prob+1*(1-prob)/prob^2),by=1)
#           }
          #draw the parent distribution plot
          plot(x=x,y=dgeom(x=x,prob=prob),main=paste(dist," Density"),
               xlab="y", ylab="f(y)",type="h")
      } else if (dist=="Poisson") {
          #parameters
          lambda<-input$param1
          ...
          #draw the parent distribution plot
          plot(x=x,y=dcauchy(x=x,location=location,scale=scale),main=paste(dist," Density"),
               xlab="y", ylab="f(y)",type="l")
      }
})
  ...
  output$statPlot <- renderPlot({
    ...
    output$statTable<-renderTable({
      ...
      samples<-simData()
      samples<-samples[N,]
      statdisplay<-data.frame(Stats=c("Sample Mean","Sample Var","Sample SD","Sample Min","Sample Median","Sample Max"),
                              Value=c(sprintf("%.4f",mean(samples)),sprintf("%.4f",var(samples)),sprintf("%.4f",sd(samples)),
          sprintf("%.4f",min(samples)),sprintf("%.4f",median(samples)),sprintf("%.4f",max(samples))))
      statdisplay
    },include.colnames=FALSE)
    ...
    output$sampStatTable<-renderTable({
      ...
      sampstatdisplay<-data.frame(Stats=c("Sample Mean","Sample Var","Sample SD","Sample Min","Sample Median","Sample Max"),Value=c(sprintf("%.4f",mean(statvals)),sprintf("%.4f",var(statvals)),sprintf("%.4f",sd(statvals))
                                ,sprintf("%.4f",min(statvals)),sprintf("%.4f",median(statvals)),sprintf("%.4f",max(statvals))))
      sampstatdisplay
    },include.colnames=FALSE)
